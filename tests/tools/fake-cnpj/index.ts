...
const fakeCnpj = () => {
  const cnpjBase = [
    faker.datatype.number({ min: 0, max: 99 }).toString().padStart(2, '0'),
    faker.datatype.number({ min: 0, max: 999 }).toString().padStart(3, '0'),
    faker.datatype.number({ min: 0, max: 999 }).toString().padStart(3, '0'),
    faker.datatype.number({ min: 1, max: 9999 }).toString().padStart(4, '0'),
  ].join('');
  const generateDV = () => {
    const m1 = [5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2]; const
      m2 = [6, 5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2];
    const numeros = cnpjBase.split('');
    ...
    + (m1[8] * parseInt(numeros[8], 10))
    + (m1[9] * parseInt(numeros[9], 10))
    + (m1[10] * parseInt(numeros[10], 10))
    + (m1[11] * parseInt(numeros[11], 10))) % 11;
    let dv = '';
    ...
    + (m2[9] * parseInt(numeros[9], 10))
    + (m2[10] * parseInt(numeros[10], 10))
    + (m2[11] * parseInt(numeros[11], 10))
    + (m2[12] * parseInt(numeros[12], 10))) % 11;
    if (resto < 2) { dv += '0'; } else { dv += (11 - resto).toString(); }
    return dv;
  };
  return `${cnpjBase}${generateDV()}`;
};

export default fakeCnpj;
